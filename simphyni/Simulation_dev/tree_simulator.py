...
class TreeSimulator:
    ...
    def _get_pair_data(self, vars: pd.DataFrame, targets: pd.DataFrame, prevalence_threshold: float = 0.00, single_trait: bool = False, batch_size: int = 2000,alpha: float = 0.05,
    ) -> Tuple[np.ndarray, np.ndarray]:
        """
        Efficiently compute Fisher’s exact test for all valid trait pairs,
        apply FDR correction, and then compute the user-defined pair statistic
        for significant pairs only.

        :param vars: Binary feature matrix (samples × traits)
        :param targets: Binary target matrix (samples × traits)
        :param prevalence_threshold: Minimum prevalence for traits to be included [0,1]
        :param single_trait: If True, only test pairs involving the first trait
        :param batch_size: Number of pairs processed per batch
        :param alpha: FDR significance threshold
        :returns: (pairs, qvals, stats) — arrays of significant pairs,
                their adjusted p-values, and custom statistic results
        """

        # Convert to NumPy for speed
        vars_np = vars.to_numpy(dtype=np.bool_)
        targets_np = targets.to_numpy(dtype=np.bool_)
        var_cols = np.array(vars.columns)
        target_cols = np.array(targets.columns)

        # Filter low-prevalence traits
        ...
        vars_np = vars_np[:, valid_vars_mask]
        targets_np = targets_np[:, valid_targets_mask]
        ...
        if single_trait:
            pairs = np.array(np.meshgrid(valid_vars[:1], valid_targets)).T.reshape(-1, 2)
        ...
        for i in range(0, len(pairs), batch_size):
            ...
            idx_vars = np.searchsorted(valid_vars, batch_pairs[:, 0])
            idx_targets = np.searchsorted(valid_targets, batch_pairs[:, 1])

            x = vars_np[:, idx_vars]
            y = targets_np[:, idx_targets]

            both = np.sum(x & y, axis=0)
            only_x = np.sum(x & ~y, axis=0)
            only_y = np.sum(~x & y, axis=0)
            ...
            vfisher = np.vectorize(fisher_p, otypes=[float])
            pvals = vfisher(both, only_x, only_y, neither)

            all_pairs.append(batch_pairs)
            all_pvals.append(pvals)

        all_pairs = np.concatenate(all_pairs, axis=0)
        all_pvals = np.concatenate(all_pvals, axis=0)

        # FDR correction
        reject, qvals,_,_ = sm.multipletests(all_pvals, alpha=alpha, method="fdr_bh")

        sig_pairs = all_pairs[reject]
        sig_qvals = qvals[reject]

        if len(sig_pairs) == 0:
            return np.array([]), np.array([])#, np.array([])

        # Compute custom pair statistics for significant pairs only
        sig_stats = []
        for i in range(0, len(sig_pairs), batch_size):
            batch_pairs = sig_pairs[i:i + batch_size]
            ...
            sig_stats.append(batch_stats)

        sig_stats = np.concatenate(sig_stats, axis=0)

        return sig_pairs, sig_stats#, sig_qvals


    def _get_pair_data_no_filt(self, vars: pd.DataFrame, targets: pd.DataFrame, prevalence_threshold: float = 0.00, single_trait: bool = False, batch_size = 1000
    ) -> Tuple[np.ndarray, np.ndarray]:
        """
    ...
    def run_simulation(self, simulation_function = None, parallel = True, bit = True, norm = True):
        ...
        if not bit:
            ...
            # Further, not all simulation implementations are suppported with bit optimization
